...
/**
 * Curiosities. What I wonder about. Reading list + verbs that link
 * out to where the rest of the thinking happens.
 */
      ...
      <ul className="mb-5 space-y-1.5">
        {reading.map((book, i) => (
          <motion.li
            ...
            className="flex items-baseline gap-2 text-sm text-ink/85"
          >
            <span className="nums text-[0.6rem] text-ink/30">0{i + 1}</span>
            <span className="font-display italic text-ink">{book.title}</span>
            <span className="text-xs text-ink/50">. {book.author}</span>
          </motion.li>
        ))}
      </ul>
        ...
        {curiosityLinks.map((l) => (
          <li key={l.href} className="flex items-baseline gap-1">
            ...
            <a
              ...
              className="group inline-flex items-baseline gap-0.5 text-ink/80 transition-colors hover:text-sky"
            >
              <span className="underline decoration-ink/15 decoration-1 underline-offset-4 group-hover:decoration-sky">
                {l.label}
              </span>
